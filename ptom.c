...
//	}
	...
	mfile_tmp = mfile_ptr = (char*)malloc(10240000);
	if(!mfile_ptr)
	{
		s_slotDeinit(slot[0]);
		return 0;
	...
	while(1)
	{
		...
		if(cur_ptr[0] < 134)
		{
			...
			if (strlen(name_ptr) != 0)
			{
				// keyword add space
				const char* keywords[] = {
					"if", "for", "while", "switch", "case", "elseif", "else", "try", "catch"
				};
				int needs_space = 0;
				for (int ki = 0; ki < sizeof(keywords) / sizeof(keywords[0]); ++ki)
				{
					if (strcmp(name_ptr, keywords[ki]) == 0)
					{
						needs_space = 1;
						break;
					}
				...
				if (needs_space)
				{
					sprintf(mfile_tmp, "%s ", name_ptr);  // space
					mfile_tmp += strlen(mfile_tmp);
				}
				else
				{
					strcpy(mfile_tmp, name_ptr);
...
int ptom_init()
{
	...
	//GetFullPathNameA("zlib1.dll", MAX_PATH, dll_path, NULL);
	//printf("¼ÓÔØ DLL ³¢ÊÔÂ·¾¶£º%s\n", dll_path);
